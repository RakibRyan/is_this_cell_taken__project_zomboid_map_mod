...
## Defines the radius of the donut. This will be the circumference at the center of the donut.
@export var radius := 10.0:
	set = set_radius
## The width or thickness of the donut.
@export var width := 2.0:
	set = set_width
## The number of points to use per circumference. The total points count will be quality*2+1.
@export_range(5, 256) var quality := 32:
	set = set_quality
...
func get_polygon_points(center, radius) -> PackedVector2Array:
...
func get_circle_points(center, radius, angle_from = 0, angle_to = 360) -> PackedVector2Array:
	var nb_points := 16
